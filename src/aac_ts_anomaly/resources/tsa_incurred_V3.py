...
verbose = True
aggreg_dimensions = ['Region', 'OE', 'Lob',  'Tier']
...
df0.rename(columns={'Line of Business': 'Lob'}, inplace=True)
...
ts_bag['RUL-UK-Property-Tier 2'].head(20)
...
gr2 = data_lev2.groupby(['Region', 'OE', 'Tier']+['time'])
...
gr2b = data_lev2_new.groupby(['Region', 'OE', 'Tier'])
...
for _, ts_info in lookup_level2_singles.iterrows():
        ...
        combi = tuple(ts_info[:len(['Region', 'OE', 'Tier'])])
        ...
        for i in range(len(combi)):
            #temp_mask *= (lookup_level1_agg[aggreg_dimensions2[i]] == combi[i])
            temp_mask &= (lookup_level1_agg[['Region', 'OE', 'Tier'][i]] == combi[i])
...
lookup_agg_1_2 = lookup_level1_agg.merge(lookup_level2_agg, how='right', left_on=['Region', 'OE', 'Tier'], right_on=['Region', 'OE', 'Tier'])
...
data_lev3 = data_lev2.merge(lookup_agg_1_2, how='right', left_on=['Region', 'OE', 'Lob','Tier'], right_on=['Region', 'OE', 'Lob','Tier']) 
data_lev3
...
gr3 = data_lev3.groupby(['Region','Tier']+['time'])
#gr3 = data_lev3.groupby(aggreg_dimensions3+['time'])
data_lev3_new = gr3.agg(target = (target_col, agg_func)).reset_index()     # aggregate

# Check cluster size again for filtering:
#-------------------------------------------
gr3b = data_lev3_new.groupby(['Region','Tier'])
...
aggr_after_lev2 = data_lev3.merge(lookup_level3_singles, how='right', left_on=['Region','Tier'], right_on=['Region','Tier']) 
...
#-------------------------------------------------------
gr4 = aggr_after_lev2.groupby(['Region', 'OE', 'Lob','Tier'])
...
for _, ts_info in lookup_level3_singles.iterrows():
        ...
        combi = tuple(ts_info[:len(['Region','Tier'])])
        ...
        temp_mask = True 
        for i in range(len(combi)):
            temp_mask &= (data_lev4[['Region','Tier'][i]] == combi[i])
...
# Next take the left-overs you have to aggregate:
#--------------------------------------------------
data_leftover = data_lev3_new.merge(lookup_level3_agg[['Region','Tier']], how='right', left_on=['Region','Tier'], right_on=['Region','Tier'])
if verbose:
...
if periodicity == 52: 
        ...
        for lc in full_agg_series['Tier'].unique():
            ts_bag['all-'+str(lc)] = full_agg_series.loc[full_agg_series['Tier'] == lc, ['time', 'target', 'year', 'month','period']]

if periodicity == 12:
        full_agg_series['year_period_ts'] = full_agg_series.apply(lambda row: _convert2date(row.year, row.period), axis=1)
        for lc in full_agg_series['Tier'].unique():
                ts_slice0 = full_agg_series.loc[full_agg_series['Tier'] == lc, ['year_period_ts', 'target']].set_index('year_period_ts', inplace=False)
...
gr0_region = df.groupby(['Region', 'Tier', 'time'])

data_region = gr0_region.agg(target = (target_col, agg_func)).reset_index()
data_region

gr_region = data_region.groupby(['Region', 'Tier'])
...
gr0_lob = df.groupby(['Lob', 'Tier', 'time'])
data_lob = gr0_lob.agg(target = (target_col, agg_func)).reset_index()
gr_lob = data_lob.groupby(['Lob', 'Tier'])
...
for _, ts_info in lookup_level_lob_singles.iterrows():
        combi = tuple(ts_info[:len(['Lob', 'Tier'])])
        ts_slice = gr_lob.get_group(combi).reset_index()

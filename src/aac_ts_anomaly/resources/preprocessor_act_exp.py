...
from claims_reporting.resources.blueprints import AbstractPreprocessor
from claims_reporting.services import file
from claims_reporting.utils.utils_func import timer
...
class claims_reporting(AbstractPreprocessor):
    ...
    """
    def __init__(self, ts_col='target', periodicity : int = 12):
    ...
    def process_data(self, data_orig : pd.core.frame.DataFrame, 
        ...
        keep_cols = ['Region', 'OE', 'Lob', 'SubLob', 'time', 'LossCat', 'Accident year',
                'Incurred amount EUR', 
                'ExpMovAttr_PAY', 'ExpMovAttr_PAY_diffs', 
                'ExpMovLg_PAY', 'ExpMovLg_PAY_diffs',
                'ExpMovMed_PAY','ExpMovMed_PAY_diffs',
                'ExpMovCat_PAY', 'ExpMovCat_PAY_diffs',
                'EUR - Plan - PAY',
                'ExpMovAttr_CAY', 'ExpMovAttr_CAY_diffs',
                'ExpMovLg_CAY', 'ExpMovLg_CAY_diffs',
                'ExpMovMed_CAY', 'ExpMovMed_CAY_diffs', 
                'ExpMovCat_CAY', 'ExpMovCat_CAY_diffs',
                'EUR - Plan - CAY']

        df = deepcopy(df0[keep_cols])
        ...
        # Use CAY if Time/Year == Accident year, else choose PAY: 
        #----------------------------------------------------------
        df['Use_CAY'] = df['time'].str[:4] == df['Accident year']

        # Make 4 Expected amount variables: 
        # 2 aggregated loss categories specific for CAY and anologously for PAY
        df['Expected Attritional'], df['Expected Medium'], df['Expected Large'], df['Expected Cat'], df['Planned'] = 0., 0., 0., 0., 0.

        # Attritional loss:
        df.loc[df['Use_CAY'], ['Expected Attritional']] = df.loc[df['Use_CAY'], ['ExpMovAttr_CAY_diffs']].values
        df.loc[~df['Use_CAY'], ['Expected Attritional']] = df.loc[~df['Use_CAY'], ['ExpMovAttr_PAY_diffs']].values
        # Medium loss
        df.loc[df['Use_CAY'], ['Expected Medium']] = df.loc[df['Use_CAY'], ['ExpMovMed_CAY_diffs']].values
        df.loc[~df['Use_CAY'], ['Expected Medium']] = df.loc[~df['Use_CAY'], ['ExpMovMed_PAY_diffs']].values
        # Large loss
        df.loc[df['Use_CAY'], ['Expected Large']] = df.loc[df['Use_CAY'], ['ExpMovLg_CAY_diffs']].values
        df.loc[~df['Use_CAY'], ['Expected Large']] = df.loc[~df['Use_CAY'], ['ExpMovLg_PAY_diffs']].values
        # Cat loss
        df.loc[df['Use_CAY'], ['Expected Cat']] = df.loc[df['Use_CAY'], ['ExpMovCat_CAY_diffs']].values
        df.loc[~df['Use_CAY'], ['Expected Cat']] = df.loc[~df['Use_CAY'], ['ExpMovCat_CAY_diffs']].values
        # Planned loss
        df.loc[df['Use_CAY'], ['Planned']] = df.loc[df['Use_CAY'], ['EUR - Plan - CAY']].values
        df.loc[~df['Use_CAY'], ['Planned']] = df.loc[~df['Use_CAY'], ['EUR - Plan - PAY']].values

        df = df.drop(columns=['ExpMovAttr_PAY', 'ExpMovAttr_PAY_diffs', 'ExpMovLg_PAY', 'ExpMovLg_PAY_diffs',
            'ExpMovMed_PAY', 'ExpMovMed_PAY_diffs', 'ExpMovCat_PAY',
            'ExpMovCat_PAY_diffs', 
            'ExpMovAttr_CAY', 'ExpMovAttr_CAY_diffs', 'ExpMovLg_CAY', 'ExpMovLg_CAY_diffs',
            'ExpMovMed_CAY', 'ExpMovMed_CAY_diffs', 'ExpMovCat_CAY',
            'ExpMovCat_CAY_diffs', 'EUR - Plan - CAY', 'EUR - Plan - PAY', 'Use_CAY'])

        # Calculate deviations, actual - expected and planned, respectively:
        #--------------------------------------------------------------------- 
        df['Dev Actual-Expected Attr'] = df['Incurred amount EUR'] - df['Expected Attritional']
        df['Dev Actual-Expected Med'] = df['Incurred amount EUR'] - df['Expected Medium']
        df['Dev Actual-Expected Lg'] = df['Incurred amount EUR'] - df['Expected Large']
        df['Dev Actual-Expected Cat'] = df['Incurred amount EUR'] - df['Expected Cat']
        df['Dev Actual-Plan'] = df['Incurred amount EUR'] - df['Planned']
        
        # Use Actual - Expected (Attritional) here first...
        self.target_col = 'Dev Actual-Expected Attr'
        ...
        sample1 = gr.agg(size=(target_col, 'count'), q_50_target=(target_col, self.q_50)).reset_index()
        ...
        data_lev2_new = gr2.agg(target = (target_col, self.agg_func)).reset_index()     # aggregate
        ...
        sample2 = gr2b.agg(size=(target_col,'count'), q_50_target=(target_col,self.q_50)).reset_index()
        ...
        #-----------------------------------------
        self.level_wise_aggr = {}       # instantiate for upcoming levels
        ...
        data_lev3_new = gr3.agg(target = (target_col, self.agg_func)).reset_index()     # aggregate

        # Check cluster size again for filtering:
        #-------------------------------------------
        gr3b = data_lev3_new.groupby(['Region','LossCat'])
        #gr3b = data_lev3_new.groupby(aggreg_dimensions3)
        sample3 = gr3b.agg(size=(target_col,'count'), q_50_target=(target_col, self.q_50)).reset_index()
        ...
        # if self.periodicity == 52: 
        #         multi_ts.index = multi_ts.apply(lambda row: self._year_week(row.year, row.period), axis=1)
        # if self.periodicity == 12:
        #         multi_ts.index = multi_ts.apply(lambda row: self._convert2date(row.year, row.period), axis=1)
        ...
        ######## !!!!!!!!!!
        # #### Hier weitermachen. Code oben passt soweit... 
        ### Alles an hier muss noch angepasst werden!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if self.aggreg_level == 'region_only':
            ...
            for _, ts_info in lookup_level_region_singles.iterrows():
                    ...
                    #ts_slice['year_period_ts'] = ts_slice.apply(lambda row: self._convert2date(row.year, row.period), axis=1)
                    ts_slice['month'] = [x.month for x in ts_slice['year_period_ts'].tolist()]
                    ts_bag_region_only['-'.join(combi)] = ts_slice[['time', self.target_col, 'year', 'month','period']]

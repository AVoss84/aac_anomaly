...
df0.rename(columns={'Line of Business': 'Lob', 'Source System': 'source_sys'}, inplace=True)
df0.shape

if pre_filter is not None:
    df0 = df0.query(pre_filter)
#df.query("Region=='South America' and OE!='Australia' and Lob=='Property'").tail(100)
df0.shape

df = deepcopy(df0)
df.dropna(inplace=True)

df['claim_creation_week']

years = df['claim_creation_week'].apply(lambda x: x[:4]).astype(int)
weeks = df['claim_creation_week'].apply(lambda x: x[4:]).astype(int)
df['claim_creation_week'] = df['claim_creation_week'].apply(lambda x: x[:4]+'-'+x[4:])
df.rename(columns={'claim_creation_week': 'time', 'Line of Business': 'Lob', 'Source System': 'source_sys'}, inplace=True)

df['year'] = df['time'].apply(lambda x: x[:4]).astype(int)
df['week'] = df['time'].apply(lambda x: x[5:]).astype(int)
...
thresh_week = max_week - ignore_week_lag
...
print("Ignoring claims younger than:",str(int(thresh_year))+'-'+'{0:02d}'.format(int(thresh_week)))

time_filter = ~((weeks > thresh_week) & (years == thresh_year))
...
weeks = weeks[time_filter]
years = years[time_filter]
week_seq = np.arange(min(weeks), max(weeks)+1)
...
for y in year_seq:
        for w in week_seq:
                time_index.append(str(int(y))+'-'+'{0:02d}'.format(int(w)))  # calendar weeks
...
# Deduplicate:
#----------------------
gr0 = df.groupby(['Lob', 'OE', 'Region','time'])
data_lev1 = gr0.agg(clm_cnt = ('clm_cnt','sum')).reset_index()
...
sample1 = gr.agg(size=('clm_cnt','count'), q_50_cnt=('clm_cnt',q_50)
                #, iqr_cnt=('clm_cnt',iqr)
                ).reset_index()
#gr = data_lev1.groupby(['Lob', 'OE', 'Region'])
...
for _, ts_info in lookup_level1_singles.iterrows():
        ...
        ts_bag['-'.join(combi)] = ts_slice[['time', 'clm_cnt', 'year', 'month','week']]
...
sample2 = gr2b.agg(size_lev2=('clm_cnt','count'), q_50_cnt_lev2=('clm_cnt',q_50)).reset_index()
sample2.shape
...
data_lev3_new = gr3.agg(clm_cnt = ('clm_cnt','sum')).reset_index()     # aggregate

# Check cluster size again for filtering:
#-------------------------------------------
gr3b = data_lev3_new.groupby(['Region'])
#sample3 = gr3b.agg(size=('Region','count')).reset_index()
sample3 = gr3b.agg(size_lev3=('clm_cnt','count'), q_50_cnt_lev3=('clm_cnt',q_50)).reset_index()
sample3.head()
...
data_lev4 = gr4.agg(size4 = ('clm_cnt','count')).reset_index()     # aggregate
...
for _, ts_info in lookup_level3_singles.iterrows():
        ...
        ts_slice['month'] = [x.month for x in ts_slice['year_week_ts'].tolist()]
        #print(ts_slice.shape[0])
        ts_bag[combi[0]] = ts_slice[['time', 'clm_cnt', 'year', 'month','week']]
...
#----------------------------------------------------
gr_full = df_orig.groupby(['time'])
full_agg_series = gr_full.agg(clm_cnt = ('clm_cnt','sum')).reset_index()
full_agg_series['year'] = full_agg_series['time'].apply(lambda x: x[:4]).astype(int)
full_agg_series['week'] = full_agg_series['time'].apply(lambda x: x[5:]).astype(int)

#from isoweek import Week
#d = Week(2020, 1).monday()
#import calendar
#calendar.MONDAY
...
#df['year_week_ts'] = df.apply(lambda row: _year_week(row.year, row.week), axis=1)
#df[['time','year', 'week', 'year_week_ts']].head(200)

full_agg_series['year_week_ts'] = full_agg_series.apply(lambda row: _year_week(row.year, row.week), axis=1)
full_agg_series['month'] = [x.month for x in full_agg_series['year_week_ts'].tolist()]
ts_bag['all'] = full_agg_series[['time', 'clm_cnt', 'year', 'month','week']]

ts_bag.get('all')
full_agg_series.tail(30)

#correct53 = (full_agg_series['month'] != 1) & (full_agg_series['week'] == 1)
#full_agg_series[correct53]

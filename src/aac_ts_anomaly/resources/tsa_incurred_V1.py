...
############ Note: This is for Expected minus Actual and/or Planned minus Actual  !!!!!!!!!!!
#############################################################################################
...
# xls = file.XLSXService(path=filename, root_path=glob.UC_DATA_DIR, dtype= {'time': str, 'Accident year': str}, sheetname='data_new', index_col=None, header=0)
...
target_col = 'Dev Actual-Expected Attr'
...
data_lev1 = gr0.agg(target = (target_col,'sum')).reset_index()
...
full_agg_series = gr_full0.agg(target = (target_col,'sum')).reset_index()
...
full_agg_series = gr_full.agg(target = (target_col,'sum')).reset_index()
full_agg_series['year'] = full_agg_series['time'].apply(lambda x: x[:4]).astype(int)
full_agg_series['period'] = full_agg_series['time'].apply(lambda x: x[5:]).astype(int)
